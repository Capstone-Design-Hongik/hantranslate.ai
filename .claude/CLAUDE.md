...
│   ├── index.ts             # 진입점, 메시지 리스너
│   ├── domExtractor.ts      # DOM 텍스트 노드 추출
│   └── domReplacer.ts       # 번역된 텍스트로 교체
├── popup/                   # Popup UI
│   ├── index.tsx            # 진입점
│   └── App.tsx              # 메인 컴포넌트
├── shared/                  # 공유 모듈
...
3. **Content Script → Background**: `TEXT_NODES` 응답
   - DOM에서 추출한 텍스트 배열 (`texts[]`) 반환
4. **Background**: 번역 처리
   - 언어 감지 (LanguageDetector API)
   - 번역 수행 (Translator API)
5. **Background → Content Script**: `REPLACE_TEXT` 메시지 전송
   - 번역된 텍스트 배열 전달
6. **Content Script**: DOM 텍스트 교체
   - 저장된 노드 참조를 사용해 텍스트 교체
7. **Background → Popup**: `STATUS` 응답 (`completed` 또는 `error`)
...
- `ContentMessage` (Background → Content)
  - `{ type: 'GET_TEXT_NODES' }`: 텍스트 노드 요청
  - `{ type: 'REPLACE_TEXT', replacements[] }`: 텍스트 교체 요청
- `ContentResponse` (Content → Background)
  - `{ type: 'TEXT_NODES', texts[] }`: 추출된 텍스트 배열
